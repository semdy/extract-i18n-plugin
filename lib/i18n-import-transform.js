...
function analyzeImportRequests(programPath, requests) {
...
function applyImportRequests(programPath, requests) {
  ...
  const { lastImportPath, requestStateMap } = analyzeImportRequests(
    programPath,
    requests
  );
  ...
  for (const requestState of requestStateMap.values()) {
    ...
    pendingInsertionsByPath.get(requestState.importPath).push(importSpecifier);
  }
  ...
  for (const [importPath, specifiers] of pendingInsertionsByPath) {
    ...
    if (insertionAnchor) {
      ...
      const [insertedPath] = programPath.unshiftContainer("body", importNode);
...
function i18nImportAstTransform(programPath, importName, importAs, importPath) {
...
function babelI18nImportTransform(programPath, options, state) {
  ...
  applyImportRequests(programPath, [...requestMap.values()]);
