...
class AITranslator extends Translator {
  constructor(option = {}) {
    super({
      ...
      fetchMethod: async (
        ...
        try {
          ...
          try {
            ...
            throw new Error("failed to parse response from big model: ", error);
          }
          if (typeof resultMap !== "object" || !resultMap) {
            throw new Error("invalid result from big model, not an object");
          }
          const isMiss = Object.keys(resultMap).some(
            key => !(key in sourceMap)
          );
          if (isMiss) {
            throw new Error("missing keys in result map from big model");
          }
          resultTextArr = textArr.map(text => resultMap[generateId(text)]); // 用textArr遍历，保证顺序
        } catch (error) {
          const message = error instanceof Error ? error.message : "unknown error";
          console.warn("⚠️", message);
          console.warn("⚠️ returned content: ", content);
          console.warn("⚠️ original text: ", JSON.stringify(sourceMap));
        }
      ...
      onError: (error, cb) => {
        cb(error);
        if (option.apiUrl.includes("volces.com")) {
          console.error(
            "❌ 请确保在火山引擎控制台开通了对应模型，且有足够的token余额。控制台地址：https://console.volcengine.com/ark/",
            error
          );
        } else {
          console.error("❌ please check your apiUrl and credentials or balance", error);
        }
      },
