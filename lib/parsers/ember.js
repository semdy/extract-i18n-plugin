...
function transformTemplate(code, options, ms, baseOffset = 0) {
  ...
  function isTranslateHelper(node) {
    return (
      node &&
      (node.type === "MustacheStatement" || node.type === "SubExpression") &&
      (node.path?.original === options.translateKey ||
        node.path?.original === options.translateKeyForTemplate)
    );
  }

  function transformExistingTranslateHelper(node) {
  ...
  function createTranslation(text) {
    ...
    if (!normalized || !shouldExtract(normalized, options)) {
      ...
      if (options.rewrite && normalized) {
        innerI18nMap[normalized] = EMPTY_PLACEHOLDER;
      }
  ...
  function buildMustacheCall(text) {
    ...
    return `${options.translateKeyForTemplate || options.translateKey} ${args}`;
  }
    ...
    switch (node.type) {
      ...
      case "MustacheStatement": {
        if (isTranslateHelper(node)) {
          transformExistingTranslateHelper(node);
          return;
        }
...
function processFile(code, options) {
  ...
  const innerI18nMapFromTemplate = {};
  ...
  if (matches.length === 0) {
    ...
    if (result.changed) {
      Object.assign(innerI18nMapFromTemplate, result.innerI18nMap);
    }
  ...
  const scriptResult = transformScript(
    strippedCode,
    options,
    innerI18nMapFromTemplate
  );
  ...
  const changed = templateChanged || scriptResult.changed;

  return {
    changed,
    code: changed && options.rewrite ? outputCode : code
  };
}
