...
function splitTopImports(code) {
  const importRegex =
    /^((?:\s*(?:\/\/[^\n]*\n|\/\*[\s\S]*?\*\/\s*|import[\s\S]+?from\s+['"][^'"]+['"];?\s*))*)/;
...
function transformTemplate(code, options, ms) {
  ...
  const parser = createParser({
    ...
    onOpenTagEnd() {
      ...
      if (currentTagInfo.msg) {
        ...
        Object.assign(innerI18nMap, {
          [id]: msg
        });
      } else if (options.rewrite && id) {
  ...
  return { changed, innerI18nMap };
}

function wrapImports(code) {
  return `${code}import 'xxxx';`;
}

function unwrapImports(code) {
  return `${code.replace("import 'xxxx';", "\n")}\n`;
}
...
function processFile(code, options) {
  ...
  if (options.rewrite && result.changed) {
    const { imports, end } = splitTopImports(code);
    const importsResult = transformScript(
      wrapImports(imports),
      options,
      result.innerI18nMap
    );
    ...
    if (end === 0) {
      ms.prepend(unwrapImports(importsResult.code));
    } else {
      ms.overwrite(0, end, unwrapImports(importsResult.code));
    }
  }
