...
import { loadConfigFile, loadConfigFileByC12 } from "./loadConfigFile.js";

const require = createRequire(import.meta.url);
const { bin } = require("../package.json");
const here = path.dirname(fileURLToPath(import.meta.url));

export class WebpackPluginI18n {
  options = {};
  ...
  loadConfigSync() {
    ...
    try {
      const config = loadConfigFile(require.resolve(configPath));
  ...
  apply(compiler) {
    ...
    const hasCustomLoader = rule =>
      ...
      rule.use.some(
        ({ loader }) => loader && loader.includes("webpack-i18n-loader")
      );
